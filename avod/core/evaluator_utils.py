...
def save_proposals_in_kitti_format(model,
                                     checkpoint_name,
                                     data_split,
                                     score_threshold,
                                     global_step):
    ...
    kitti_predictions_3d_dir = predictions_root_dir + '/kitti_proposals'
    ...
    print('3D Proposals being saved to:', kitti_predictions_3d_dir)
    ...
    for sample_idx in range(num_samples):
        ...
        for i in range(len(all_predictions)):
            ...
            box_3d = np.append(all_predictions[i, 0:6], 0)
        ...
        # Get object types
        all_pred_classes = all_predictions[:, 6].astype(np.int32)
        ...
        # (ry, score)
        kitti_predictions[:, 14] = 0
        kitti_predictions[:, 15] = all_predictions[:, 7]
...
def save_predictions_in_kitti_format(model,
    ...
    for sample_idx in range(num_samples):
        ...
        all_predictions = np.reshape(np.loadtxt(predictions_file_path), (-1, 9))
