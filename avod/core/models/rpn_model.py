...
class RpnModel(model.DetectionModel):
    ...
    SCORE_2D_THRES_MORE = 0.01

    SCORE_2D_GT = 'score_2d_gt'
    PRED_SCORE_2D = 'pred_score_2d'

    PL_IMG_INPUT = 'img_input_pl'
    PL_IMG_R_INPUT = 'img_r_input_pl'
    ...
    PL_IMG_R_ANCHORS = 'img_r_anchors_pl'
    PL_IMG_R_ANCHORS_NORM = 'img_r_anchors_norm_pl'
    ...
    PRED_MB_POS_GT = 'rpn_mb_pos_gt'
    PRED_MB_NEG_GT = 'rpn_mb_neg_gt'
    ...
    LOSS_RPN_SCORE_2D = 'rpn_score_2d_loss'
    ...
    def _print_tensor_shape(self, x, name):
    ...
    def _set_up_input_pls(self):
        ...
        with tf.variable_scope('img_r_input'):
            # Take variable size input images
            img_r_input_placeholder = self._add_placeholder(
                tf.float32,
                [None, None, self._img_depth],
                self.PL_IMG_R_INPUT)

            self._img_r_input_batches = tf.expand_dims(
                img_r_input_placeholder, axis=0)

            self._img_r_preprocessed = \
                self._img_feature_extractor.preprocess_input(
                    self._img_r_input_batches, self._img_pixel_size)

            # Summary Image
            tf.summary.image("rgb_image_r", self._img_r_preprocessed,
                             max_outputs=2)
        ...
        with tf.variable_scope('pl_labels'):
            ...
            self._add_placeholder(tf.float32, [None, None, 2], 
                                  self.SCORE_2D_GT)
        ...
        with tf.variable_scope('pl_anchors'):
            ...
            with tf.variable_scope('img_r_anchor_projections'):
                self._add_placeholder(tf.float32, [None, 4],
                                      self.PL_IMG_R_ANCHORS)
                self._img_r_anchors_norm_pl = self._add_placeholder(
                    tf.float32, [None, 4], self.PL_IMG_R_ANCHORS_NORM)
            ...
            with tf.variable_scope('sample_info'):
                ...
                self._add_placeholder(
                    tf.float32, [3, 4], self.PL_CALIB_P3)  
    ...
    def _set_up_feature_extractors(self):
        ...
        with tf.variable_scope('img_feature_extractor'):
            """
            ...
            self.img_r_feature_maps, self.img_r_end_points = \
            self._img_r_feature_extractor.build(
                self._img_r_preprocessed,
                self._img_pixel_size,
                self._is_training)
            """

            self.img_lr_feature_maps, self.pred_lr_score_2d, self.img_end_points = \
            self._img_feature_extractor.build(
                tf.concat([self._img_preprocessed,
                           self._img_r_preprocessed], axis=0),
                self._img_pixel_size,
                self._is_training)

            self.img_feature_maps, self.img_r_feature_maps = \
                tf.split(self.img_lr_feature_maps, [1, 1], axis=0)

            self.pred_score_2d, self.pred_r_score_2d = \
                tf.split(self.pred_lr_score_2d, [1, 1], axis=0)

            tf.add_to_collection('pred_score_2d', self.pred_score_2d)
        ...
        with tf.variable_scope('img_bottleneck'):
            ...
            self.img_r_bottleneck = slim.conv2d(
                self.img_r_feature_maps,
                32, [1, 1],
                scope='bottleneck',
                normalizer_fn=slim.batch_norm,
                normalizer_params={
            ...
            self.img_lr_bottleneck = slim.conv2d(
                self.img_lr_feature_maps,
                4, [1, 1],
                scope='bottleneck',
                normalizer_fn=slim.batch_norm,
                normalizer_params={
            ...
            self.img_bottleneck, self.img_r_bottleneck = \
                 tf.split(self.img_lr_bottleneck, [1, 1], axis=0)
    ...
    def build(self):
        ...
        img_r_proposal_input = self.img_r_bottleneck

        pred_score_2d_softmax = tf.nn.softmax(self.pred_score_2d)
        score_b, score_h, score_w, score_c = pred_score_2d_softmax.get_shape().as_list()
        assert score_b == 1 and score_h == 12 and score_w == 39 and score_c == 2
        
        y1x1, y2x2 = tf.split(tf.clip_by_value(self._img_anchors_norm_pl, 0, 1-1e-5), [2, 2], axis=1)
        all_anchors_2d_center_norm = (y1x1 + y2x2) * 0.5
        all_anchors_2d_center = all_anchors_2d_center_norm * \
                                tf.constant(value=(score_h, score_w), 
                                            dtype=tf.float32, 
                                            shape=(1, 2))
        all_anchors_2d_center = tf.cast(all_anchors_2d_center, tf.int32)
        all_anchors_2d_score = tf.gather_nd(pred_score_2d_softmax[0, :, :, 1], \
                                            all_anchors_2d_center)

        keep = tf.greater_equal(all_anchors_2d_score, self.SCORE_2D_THRES)

        keep_more = tf.greater_equal(all_anchors_2d_score, self.SCORE_2D_THRES_MORE)

        """
        keep_ori.set_shape((self.NUM_ANCHORS, ))
        keep_non_ept = tf.concat([tf.constant(True, dtype=tf.bool, shape=(1, )), keep_ori], axis=0)[:-1]
        """
        keep = tf.cond(tf.reduce_any(keep), lambda: keep, lambda: keep_more)
    
        self._keep = keep

        self._img_anchors_norm_pl = tf.boolean_mask(self._img_anchors_norm_pl, keep)
        self._img_r_anchors_norm_pl = tf.boolean_mask(self._img_r_anchors_norm_pl, keep)
        ...
        if not (self._path_drop_probabilities[0] ==
            ...
            with tf.variable_scope('rpn_path_drop'):
                ...
                img_mask, img_r_mask = self.create_path_drop_masks(
                    self._path_drop_probabilities[0],
                    self._path_drop_probabilities[1],
                    random_values)
                """
                ...
                img_r_proposal_input = tf.multiply(img_r_proposal_input,
                                                 img_r_mask)

                self.img_path_drop_mask = img_mask
                self.img_r_path_drop_mask = img_r_mask

                # Overwrite the division factor
                fusion_mean_div_factor = img_mask + img_r_mask
        ...
        with tf.variable_scope('proposal_roi_pooling'):
            ...
            with tf.variable_scope('box_indices'):
                ...
                img_boxes_norm_batches = tf.expand_dims(
                    self._img_anchors_norm_pl, axis=0)
            ...
            img_r_proposal_rois = tf.image.crop_and_resize(
                img_r_proposal_input,
                self._img_r_anchors_norm_pl,
                tf_box_indices,
                (16, 16))


            img_proposal_rois = self._norm(img_proposal_rois)
            img_r_proposal_rois = self._norm(img_r_proposal_rois)

            cos_simi = tf.reduce_sum(img_proposal_rois * img_r_proposal_rois, \
                                     axis=[1, 2], keep_dims=True)

            cos_simi = tf.nn.relu(cos_simi)

           
            img_proposal_rois = tf.image.resize_bilinear(img_proposal_rois, \
                                                         self._proposal_roi_crop_size)
            img_r_proposal_rois = tf.image.resize_bilinear(img_r_proposal_rois, \
                                                           self._proposal_roi_crop_size)
        ...
        with tf.variable_scope('proposal_roi_fusion'):
            ...
            if self._fusion_method == 'mean':
                tf_features_sum = tf.add(img_proposal_rois, img_r_proposal_rois)
                rpn_fusion_out = tf.divide(tf_features_sum,
                                           fusion_mean_div_factor) * cos_simi
            else:
        ...
        with tf.variable_scope('histograms_feature_extractor'):
            ...
            with tf.variable_scope('img_vgg'):
                ...
                for end_point in self.img_r_end_points:
                    tf.summary.histogram(
                        end_point, self.img_r_end_points[end_point])
        ...
        with tf.variable_scope('proposals'):
            ...
            anchors = tf.boolean_mask(self.placeholders[self.PL_ANCHORS], \
                                      self._keep)
        ...
        all_ious_gt = tf.boolean_mask(self.placeholders[self.PL_ANCHOR_IOUS], self._keep)
        all_offsets_gt = tf.boolean_mask(self.placeholders[self.PL_ANCHOR_OFFSETS], self._keep)
        all_classes_gt = tf.boolean_mask(self.placeholders[self.PL_ANCHOR_CLASSES], self._keep)
        ...
        with tf.variable_scope('img_r_rpn_rois'):
            # ROIs on image input
            mb_img_r_anchors_norm = tf.boolean_mask(self._img_r_anchors_norm_pl,
                                                    mini_batch_mask)
            mb_img_r_box_indices = tf.zeros_like(
                tf.boolean_mask(all_classes_gt, mini_batch_mask),
                dtype=tf.int32)
            ...
            tf.summary.image('img_r_rpn_rois',
                             img_r_input_rois,
                             max_outputs=rpn_mini_batch_size)
        ...
        with tf.variable_scope('one_hot_classes'):
            ...
            neg_gt = tf.cast(
                tf.less(all_ious_gt, max_neg_iou),
                dtype=tf.float32)
        ...
        with tf.variable_scope('ground_truth_mini_batch'):
            ...
            pos_gt_masked = tf.boolean_mask(
                pos_gt, mini_batch_mask)

            neg_gt_masked = tf.boolean_mask(
                neg_gt, mini_batch_mask)
        ...
        if self._train_val_test in ['train', 'val']:
            ...
            predictions[self.PRED_MB_POS_GT] = pos_gt_masked
            predictions[self.PRED_MB_NEG_GT] = neg_gt_masked
            ...
            predictions[self.PRED_SCORE_2D] = self.pred_score_2d
            predictions[self.SCORE_2D_GT] = self.placeholders[self.SCORE_2D_GT]
    ...
    def create_feed_dict(self, sample_index=None):
        ...
        label_score_2d = sample.get(constants.KEY_LABEL_SCORE_2D)
        ...
        image_r_input = sample.get(constants.KEY_IMAGE_R_INPUT)
        ...
        self._placeholder_inputs[self.PL_IMG_R_INPUT] = image_r_input
        ...
        self._placeholder_inputs[self.SCORE_2D_GT] = label_score_2d

        # Sample Info
        # img_idx is a list to match the placeholder shape
        self._placeholder_inputs[self.PL_IMG_IDX] = [int(sample_name)]
        self._placeholder_inputs[self.PL_CALIB_P2] = stereo_calib_p2
        self._placeholder_inputs[self.PL_CALIB_P3] = stereo_calib_p3
    ...
    def _fill_anchor_pl_inputs(self,
        ...
        img_r_anchors, img_r_anchors_norm = \
             anchor_projector.project_to_image_space(
        ...
        self._img_r_anchors_norm = img_r_anchors_norm[:, [1, 0, 3, 2]]
        ...
        self._placeholder_inputs[self.PL_IMG_R_ANCHORS] = img_r_anchors
        self._placeholder_inputs[self.PL_IMG_R_ANCHORS_NORM] = \
            self._img_r_anchors_norm
    ...
    def loss(self, prediction_dict):
        ...
        score_2d_gt = tf.reshape(prediction_dict[self.SCORE_2D_GT], (-1, 2))

        pos_gt = prediction_dict[self.PRED_MB_POS_GT]
        neg_gt = prediction_dict[self.PRED_MB_NEG_GT]
        ...
        with tf.variable_scope('rpn_prediction_mini_batch'):
            ...
            score_2d = tf.reshape(prediction_dict[self.PRED_SCORE_2D], (-1, 2))

        with tf.variable_scope('rpn_losses'):
            with tf.variable_scope('score_2d'):
                cls_loss = losses.WeightedSoftmaxLossPiecewise()
                cls_loss_weight = self._config.loss_config.cls_loss_weight
                score_loss, acc_score_neg, acc_score_pos = cls_loss._compute_loss(score_2d, 
                                                                      score_2d_gt, 
                                                                      score_2d_gt[:, 1], 
                                                                      score_2d_gt[:, 0],
            ...
            with tf.variable_scope('objectness'):
                cls_loss = losses.WeightedSoftmaxLossPiecewise()
                cls_loss_weight = self._config.loss_config.cls_loss_weight
                objectness_loss, acc_all, acc_pos = cls_loss._compute_loss(objectness,
                                           objectness_gt,
                                           pos_gt, 
                                           neg_gt,
                                           weight=cls_loss_weight)
            ...
            with tf.variable_scope('regression'):
                ...
                with tf.variable_scope('reg_norm'):
                    ...
                    num_positives = tf.reduce_sum(objectness_gt[:, 1]) + 1e-2
            ...
            with tf.variable_scope('total_loss'):
                ...
                if self.TRAIN_SCORE_2D and not self.TRAIN_RPN_3D:
                    total_loss = score_loss
                elif not self.TRAIN_SCORE_2D and self.TRAIN_RPN_3D:
                    total_loss = objectness_loss + localization_loss
                elif self.TRAIN_SCORE_2D and self.TRAIN_RPN_3D:
                    total_loss = score_loss + objectness_loss + localization_loss
                else:
                    total_loss = 0.0

        loss_dict = {
            self.LOSS_RPN_SCORE_2D: score_loss,
            self.LOSS_RPN_OBJECTNESS: objectness_loss,
            self.LOSS_RPN_REGRESSION: localization_loss,
        }

        return loss_dict, total_loss, acc_all, acc_pos, acc_score_neg, acc_score_pos
