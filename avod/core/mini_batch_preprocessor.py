...
class MiniBatchPreprocessor(object):
    ...
    def _calculate_anchors_info(self,
        ...
        for gt_idx in range(len(gt_labels)):
            ...
            if self.mini_batch_utils.rpn_iou_type == '2d':
                ...
                ious = obj_utils.two_d_iou(gt_box_for_2d_iou,
                                            anchors_for_2d_iou)
            elif self.mini_batch_utils.rpn_iou_type == '3d':
                gt_box_for_3d_iou = gt_boxes_for_3d_iou[gt_idx]
                ious = obj_utils.three_d_iou(gt_box_for_3d_iou,
                                              anchors_for_3d_iou)
    ...
    def preprocess(self, indices):
        ...
        for sample_idx in indices:
            ...
            #empty_anchor_filter = self._keep_near_anchors(all_anchor_boxes_3d, filtered_gt_list, 30)
            empty_anchor_filter = np.ones(all_anchor_boxes_3d.shape[0]).astype(np.bool)
