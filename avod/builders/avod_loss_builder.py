...
def _get_cls_loss(model, cls_logits, cls_gt, pos_gt, neg_gt):
    ...
    weighted_softmax_classification_loss = \
        losses.WeightedSoftmaxLossPiecewise()
    cls_loss_weight = model._config.loss_config.cls_loss_weight
    cls_loss, acc_all, acc_pos = weighted_softmax_classification_loss._compute_loss(
        cls_logits, cls_gt, pos_gt, neg_gt, cls_loss_weight)
    ...
    return cls_loss, acc_all, acc_pos
...
def _get_off_ang_loss(model, offsets, offsets_gt,
    ...
    return final_reg_loss * 0.1, offset_loss_norm * 0.1, ang_loss_norm * 0.1
